...
class CASServer::Authenticators::SQL < CASServer::Authenticators::Base
  ...
  def validate(credentials)
    ...
    $LOG.debug "#{self.class}: [#{user_model}] " + "Connection pool size: #{user_model.connection_pool.instance_variable_get(:@size)}/#{user_model.connection_pool.instance_variable_get(:@size)}"
